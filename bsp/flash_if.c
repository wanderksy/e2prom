...
#if 0
uint32_t FLASH_EraseOneSectors(uint32_t Address)
{
  /* Erase the user Flash area
	...
	FLASH_Erase_Sector(Sector, Banks, FLASH_VOLTAGE_RANGE_3);
...
uint32_t FLASH_EraseOneSectors(uint32_t Address)
{
  ...
  if(Address <= (uint32_t) USER_FLASH_END_ADDRESS)
  {
	Sector = GetSector(Address);	
	if(Address < ADDR_FLASH_SECTOR_0_BANK2)
		Banks = FLASH_BANK_1;
	...
	FLASH_EraseInitStruct.Banks = Banks;
...
uint32_t FLASH_write(uint32_t Address, void * p_dat , uint32_t len)
{
	uint32_t * p_dat32=(uint32_t *)p_dat;
	uint32_t	len32=len>>2;
	
	FLASH_If_Write(Address,  p_dat32 , len32);
